...
vector<float> initialize_priors(int map_size, vector<float> landmark_positions,
  ...
  // TODO: YOUR CODE HERE
  float di = landmark_positions.size()*landmark_positions.size();
    
    for (int i = 0; i<landmark_positions.size(); ++i) {
        for (int j=1; j<map_size; ++j) {
            if (landmark_positions[i] == j) {
                //std::cout << i << std::endl;
                //std::cout << j << std::endl;
                priors[j-position_stdev] += 1/di;
                priors[j] += 1/di;
                priors[j+position_stdev] += 1/di;
            }
        }
 ...
 float j = 1;
  ...
  for (int i=0; i < landmark_positions.size(); ++i) {
      priors.at(int(j+landmark_positions[i])) += 1.0/norm_term;
      priors.at(int(-j+landmark_positions[i])) += 1.0/norm_term;
